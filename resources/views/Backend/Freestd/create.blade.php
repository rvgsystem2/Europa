<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto bg-white shadow-lg rounded-xl p-8 mt-10">
        <h2 class="text-2xl font-bold text-gray-800 mb-6 text-center">Add New freestd</h2>

        <form action="{{ route('freestd.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf

            <div>
               <label class="block text-sm font-medium text-gray-700 mb-2">freestd Title</label>
              <input type="text" placeholder="Enter freestd title"
                     name="title"
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Upload freestd Image</label>
                <input
                    type="file"
                    name="image"
